...
export default async function PayslipPrintPage({
  ...
  // Fetch payslip
  const payslipResult = await db.execute({
  ...
  // Compute deductions for display (use stored or fallback to employee config)
  ...
  const netPay = payslip.net_amount;
  ...
  return (
    <html lang="sw">
      <head>
        ...
        <title>
          Kielelezo cha Mshahara — {employee?.name ?? id} — {periodLabel}
        </title>
        <style>{`
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .pay-table td:last-child, .pay-table th:last-child {
            text-align: right;
          }
          .pay-table .subtotal td {
            font-weight: 600;
            background: #fafafa;
          }
          .net-pay-row {
      ...
      <body>
        ...
        <div className="info-grid">
          <div className="info-item">
            <span className="lbl">Jina:</span>
            <span className="val">{employee?.name ?? "—"}</span>
          </div>
          <div className="info-item">
            <span className="lbl">Kipindi:</span>
            <span className="val">{periodLabel}</span>
          </div>
          <div className="info-item">
            <span className="lbl">Nambari ID:</span>
            <span className="val">{payslip.employee_id}</span>
          </div>
          <div className="info-item">
            <span className="lbl">Simu:</span>
            <span className="val">{employee?.phone ?? "—"}</span>
          </div>
          <div className="info-item">
            <span className="lbl">Sehemu:</span>
            <span className="val">{section?.name ?? "—"}</span>
          </div>
          <div className="info-item">
        ...
        <table className="pay-table">
          ...
          <tbody>
            {isFulltime ? (
              <tr>
                <td>Mshahara wa Mwezi</td>
                <td>{formatCurrency(employee?.monthly_salary ?? payslip.gross_amount)}</td>
              </tr>
            ) : (
              <>
                <tr>
                  <td>Siku Zilizofanywa Kazi</td>
                  <td>{payslip.days_worked}</td>
                </tr>
                <tr>
                  <td>Kiwango cha Siku</td>
                  <td>{formatCurrency(employee?.daily_rate ?? 0)}</td>
                </tr>
                <tr>
                  <td>
            ...
            <tr className="subtotal">
              <td>Jumla ya Mapato (Gross)</td>
              <td>{formatCurrency(payslip.gross_amount)}</td>
            </tr>
          </tbody>
        </table>

        {/* Deductions — fulltime only or if there are any */}
            ...
            <table className="pay-table">
              ...
              <tbody>
                {nssfAmt > 0 && (
                  <tr>
                    <td>NSSF</td>
                    <td>{formatCurrency(nssfAmt)}</td>
                  </tr>
                )}
                {cotwuAmt > 0 && (
                  <tr>
                    <td>COTWU</td>
                    <td>{formatCurrency(cotwuAmt)}</td>
                  </tr>
                )}
                {fadhilaAmt > 0 && (
                  <tr>
                    <td>Fadhila</td>
                    <td>{formatCurrency(fadhilaAmt)}</td>
                  </tr>
                )}
                {heslbAmt > 0 && (
                  <tr>
                    <td>HESLB</td>
                    <td>{formatCurrency(heslbAmt)}</td>
                  </tr>
                )}
                {totalAdvances > 0 && (
                  <tr>
                    <td>Mikopo (Advances)</td>
                ...
                <tr className="subtotal">
                  ...
                  <td>{formatCurrency(computedTotalDeductions)}</td>
                </tr>
              </tbody>
            </table>
          </>
        ...
        <table className="net-pay-row">
          <tbody>
            <tr>
              ...
              <td className="net-pay-amount">{formatCurrency(netPay)}</td>
            </tr>
          </tbody>
        </table>
